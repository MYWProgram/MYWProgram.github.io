...
浅拷贝：创建一个新对象，这个对象有着原始对象属性值的一份精确拷贝；**如果属性值是基本类型，拷贝的就是基本类型的值；如果属性是复杂类型，拷贝的就是内存地址**，所以二者任一对象做出了修改，都会影响到另一个对象。
...
可见，`Object.assign()`不仅可以用于浅拷贝的实现，也可以用于非重名键情况下的对象合并。
...
{
  name: "Mike"
  friends: {male: ["Jerry", "DaMing", "Bob"], female: "Lily"}
  hobby: ["basketball", "football", "tennis"]
}
*/
```

浅拷贝的情况下，由于复杂类型数据的拷贝实际是拷贝了引用地址，所以改变一个对象的复杂类型，另外一个也会受到影响，这证实了上面所说的；同时在对象中有数组的情况下也不会有异常出现；下面看深拷贝。
...
从输出结果来看，修改深层复杂类型数据，不论是对象还是数组，都不会对深克隆对象产生任何影响；但是当**循环引用**时，就会产生异常了：

在上面的例子中，定义 p 之前加上代码进行循环引用`person.person = person;`，就会报错；怎么解决这个问题呢，继续向下探究。

### 基础版本

参考上面使用`for...in`的浅拷贝代码，我们加上递归的代码，然后稍作以下修改：
...
仔细看输出结果，深拷贝的对象中 friends.male、hobby 数组变成了一个对象，这是由于深拷贝函数中直接定义 result 为空对象造成的，需要在定义的时候作出判断：
...
从输出结果可以看出，深拷贝的对象中包含了对源对象的全引用，即 person 类型 Circular，循环引用；当然报错信息也被解决了。
