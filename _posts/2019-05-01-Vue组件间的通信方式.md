...
由于两种方法都是直接获取到组件的实例，可能会出现一子多父、一父多子的情况（这种情况下，打印值会是一个数组，数组中包含按照 DOM 结构排序的实例对象），这样就很容易出错；所以该方法更适用于单一的**父子组件**关系。
...
通过上图可以看到，使用`$parent/$children`分别访问到了父组件和自组件的实例。
...
父组件 A 通过 props 的方式向子组件 B 传递，相反，B -> A 通过在 B 组件中使用`$emit`，在 A 组件中 v-on 监听实现。
